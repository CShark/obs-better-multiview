...
namespace Interceptor {
    public class Input {
        ...
        private void DriverCallback() {
            ...
            while (InterceptionDriver.Receive(context, deviceId = InterceptionDriver.Wait(context), ref stroke, 1) >
                   0) {
                if (InterceptionDriver.IsMouse(deviceId) > 0) {
                    if (OnMousePressed != null) {
                        var args = new MousePressedEventArgs() {
                            ...
                            Rolling = stroke.Mouse.Rolling, DeviceId = deviceId
                        };
                ...
                if (InterceptionDriver.IsKeyboard(deviceId) > 0) {
                    if (OnKeyPressed != null) {
                        var args = new KeyPressedEventArgs()
                            {Key = stroke.Key.Code, State = stroke.Key.State, DeviceId = deviceId};
